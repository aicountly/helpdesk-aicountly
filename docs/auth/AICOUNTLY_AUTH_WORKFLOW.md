# AICOUNTLY auth workflow (Helpdesk)

How Helpdesk signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `helpdesk.aicountly.com` (or `helpdesk.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/helpdesk?returnUrl={origin}/auth/callback`.
...
| Helpdesk host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `helpdesk.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `helpdesk.aicountly.com/api` |
| `helpdesk.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `helpdesk.gh.aicountly.com/api` |
...
`helpdesk` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://helpdesk.gh.aicountly.com/api/health
...
curl -i -X POST https://helpdesk.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://helpdesk.gh.aicountly.com/api/global/login   # expect 404
```
